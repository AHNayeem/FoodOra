...
function announce(subscription: Subscription) {
  ...
  persist(
    (set, get) => ({
      ...
      replace: (subscription) => {
        const before = get().subscriptions.find((s) => s.id === subscription.id);
        set((s) => ({
          subscriptions: s.subscriptions.map((existing) =>
            existing.id === subscription.id ? subscription : existing,
          ),
        }));
        // A skipped day is not a state change — the plan is still active, and a
        // notification for every skip would drown the one that matters (paused,
        // resumed, cancelled). Pausing again to a *different* resume date is,
        // which is why the notification id carries `pausedUntil`.
        if (
          before?.status !== subscription.status ||
          before?.pausedUntil !== subscription.pausedUntil
        ) {
          announce(subscription);
        }
      },
