/** Meal-plan directory loading skeleton (Phase C15). */
export default function Loading() {
  return (
    <div className="pb-16">
      <div className="border-b border-line bg-surface-alt">
        <div className="container-site py-12 md:py-16">
          <div className="h-6 w-32 animate-pulse rounded-pill bg-surface-muted" />
          <div className="mt-4 h-10 w-2/3 max-w-lg animate-pulse rounded-field bg-surface-muted" />
          <div className="mt-3 h-5 w-80 max-w-full animate-pulse rounded-field bg-surface-muted" />
        </div>
      </div>
      <div className="container-site mt-10">
        ...
        <div className="mt-4 flex gap-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-10 w-28 animate-pulse rounded-pill bg-surface-muted" />
          ))}
        </div>
