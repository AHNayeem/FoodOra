...
export function AccountShell({ children }: { children: React.ReactNode }) {
  ...
  // The one nav entry that carries a count. Read here rather than inside the
  // link so the sidebar stays a single subscription.
  const notifications = useNotifications((s) => s.items);
  const notificationsHydrated = useNotifications((s) => s.hydrated);
  const unread = notificationsHydrated ? unreadCount(notifications, "customer") : 0;
                ...
                <li key={href} className="shrink-0">
                  ...
                  >
                    ...
                    {key === "notifications" && unread > 0 && (
                      <span className="ms-auto inline-flex min-w-5 items-center justify-center rounded-pill bg-danger px-1.5 text-[11px] font-bold text-white tabular-nums">
                        {unread > 99 ? "99+" : unread}
                      </span>
                    )}
                  </Link>
