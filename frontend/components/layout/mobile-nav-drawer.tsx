...
export function MobileNavDrawer({
  ...
  onClose: () => void;
  user: User | null;
  /** The CMS `header` menu (C26) — the same list the desktop bar renders. */
  menu: CmsMenuItem[];
}) {
    ...
    <div className="fixed inset-0 z-[70] lg:hidden">
      ...
      >
        ...
        <nav aria-label={t("nav.primary")} className="flex-1 overflow-y-auto px-3 py-4">
          {navGroups
            .filter((group) => items.some((item) => item.group === group.id))
            .map((group) => (
                    ...
                    return (
                      <li key={item.id}>
                        ...
                        >
                          ...
                          >
                            <DashIcon name={item.icon} className="size-4.5" />
                          </span>
                          {item.label}
                        </Link>
                      </li>
