...
export function PrimaryNav({ className }: { className?: string }) {
  ...
  const measure = useCallback(() => {
    ...
    if (nodes.length !== primaryNav.length + 1) return; // items + More trigger
    ...
    while (count > 0) {
      const items = widths.slice(0, count).reduce((sum, w) => sum + w, 0) + gap * (count - 1);
      const trigger = count < widths.length ? gap + moreWidth : 0;
      if (items + trigger <= available) break;
      count--;
    }
  ...
  const shown = primaryNav.slice(0, visibleCount);
  const overflow = primaryNav.slice(visibleCount);
  ...
  return (
    ...
    >
              ...
              >
                {overflow.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={cn("size-4", !active && "text-muted")} aria-hidden />
                      {t(item.labelKey)}
                    </Link>
                  );
                })}
      ...
      >
        {primaryNav.map((item) => (
          <li key={item.href}>
            <span className={cn(linkClass, restClass)}>{t(item.labelKey)}</span>
          </li>
        ))}
        <li>{moreTrigger}</li>
      </ul>
