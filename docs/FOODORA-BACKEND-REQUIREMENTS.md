...
| 5 | Menu & inventory | 4 | **done** — availability derived as merchant switch AND (untracked OR in stock); stock is a balance plus an append-only movement log, moved by one guarded `UPDATE`. [M5](./backend/M5-menu-inventory.md) |
